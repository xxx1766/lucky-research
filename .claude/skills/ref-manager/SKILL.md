...
2. For each slug, look up the matching paper in AgentDB `papers/<slug>` to get bibinfo.
3. Emit BibTeX entries into `outputs/references/<paper-slug>.bib` via
   `research_assistant.refs.merge_bibtex` (dedupes by DOI/title).
4. Replace placeholders with the BibTeX citation key (e.g. `\cite{smith2024}`).
...
- `outputs/references/<paper-slug>.bib`
