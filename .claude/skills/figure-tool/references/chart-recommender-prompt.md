# Chart-recommender prompt — pick the right academic chart from data shape

> **Source:** Adapted from
> [Leey21/awesome-ai-research-writing](https://github.com/Leey21/awesome-ai-research-writing)
> (no explicit LICENSE; public awesome-list intended for sharing).
> The Role / Constraints / Output Format blocks and the 19-chart library
> below are reproduced verbatim from that README's "实验绘图推荐" entry.

## How `/figure recommend` uses this

1. Read this file as the prompt prelude.
2. Resolve the current scope (paper or experiment cursor) — recommendations
   should still know which palette is in play.
3. Collect inputs from the user (plain text, no AskUserQuestion per the
   `feedback_decision_ui` memory):
   - **Data snippet** — paste an Excel/CSV table, or describe the data shape
     in 2–3 lines (variables, axes, sample count, distribution shape).
   - **Core conclusion** — one sentence on what the figure must demonstrate
     ("show our method's Pareto-optimal accuracy/latency tradeoff", "expose
     the long-tail of failure-mode types", …).
4. **Apply** the prompt below with both inputs stitched into the `# Input`
   block.
5. The model returns the four-section structured output:
   - 推荐方案 (1–2 chart names from the library)
   - 核心理由 (why these match the academic narrative)
   - 视觉设计规范 (axes, scale handling, statistical elements, palette)
6. Print the response to the user, then offer the natural hand-off:
   - `Next: /figure new <slug>` — start the 6-step generation with
     `kind=data` and the recommended chart as Step-2 hint.
   - The user keeps editorial control; we never auto-spawn `/figure new`.
...
```
建议下一步: /figure new <slug>
  → kind=data, chart=<推荐方案>, palette=<当前 direction 默认>
```

The user types the command themselves with their chosen slug. `/figure new`
re-validates the chart kind in Step 2; the recommender's chart name is a
hint, not a binding.
